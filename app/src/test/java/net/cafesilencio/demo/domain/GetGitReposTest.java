...
public class GetGitReposTest {
    ...
    @Test
    public void call() {
        server.enqueue(new MockResponse().setBody(successFullCallResp));

        HttpUrl baseUrl = server.url("/repositories");

        when(gitServiceConfig.getGitRepoUrl()).thenReturn(baseUrl.toString());

        List<GitRepoRecord> items = getGitRepos.call().blockingGet();

        assertThat(items.size(), is(2));
        assertThat(items.get(0).getId(), is(1));
        assertThat(items.get(1).getId(), is(26));
    }
}
